...
className="w-4 h-4"
 ...
 <button  onClick={showAdd}  className="bg-yellow-500 text-white px-4 py-2 rounded-lg"  >  + Add a todo</button>
 <h2 className="text-xl font-semibold mt-8 mb-3">Things done</h2>
  <div className="space-y-2">
 {done.map((d)=>(
    <div key={d.id} className="flex items-center gap-2">
<input  type="checkbox" checked className="w-4 h-4" onChange={() => toggleTodo(d.id)}  />
